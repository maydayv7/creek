# -*- coding: utf-8 -*-
"""inpainting (1) (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15HZcoDGdzTaDO9d1uIUMI8LQToCP4ryC
"""
...
def generate_and_save_mask(clean_path, drawn_path, save_path):
    print("🛠️ Generating mask with robust filtering...")
...
if __name__ == "__main__":
    success = generate_and_save_mask(CLEAN_IMAGE_PATH, DRAWN_IMAGE_PATH, MASK_OUTPUT_PATH)
    if success:
...
def run_sketch_to_image(pipe, clean_path, drawn_path, prompt, negative_prompt="", strength=0.85, seed=42):
    if not os.path.exists(clean_path) or not os.path.exists(drawn_path):
    ...
    axs[0].imshow(img_clean); axs[0].set_title("Initial")
    axs[1].imshow(img_drawn); axs[1].set_title("Sketch")
    axs[2].imshow(mask, cmap="gray"); axs[2].set_title("Calculated Mask (Fixed)")
    axs[3].imshow(output); axs[3].set_title("Result")
    plt.tight_layout()
    plt.show()
    return output
...
def generate_concise_caption(image_path):
    ...
    inputs = processor(text=prompt_task, images=image, return_tensors="pt").to(device, dtype)
...
text_output = generate_concise_caption("/kaggle/input/test1/tree_sktech.jpg")
