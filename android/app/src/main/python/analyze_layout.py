...
def full_bleed_score(
    ...
    cond_bbox_touch = (
        bbox_touch >= bbox_touch_required
    )  # e.g. >=0.75 means touching 3 or 4 edges
...
def analyze_single_image(img_path):
    if not os.path.exists(img_path):
        return json.dumps(
            {"scores": {}, "error": f"File not found at {img_path}"}, cls=NumpyEncoder
        )

    img_full = cv2.imread(img_path)
    if img_full is None:
        return json.dumps(
            {"scores": {}, "error": f"Could not load image. Check format."},
            cls=NumpyEncoder,
        )
    ...
    result = {"scores": composition_scores, "diagnostics": diagnostics}
